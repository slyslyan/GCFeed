...
5. 这个项目最能体现后端能力的 2-3 个设计点是什么？
   ...
   答：第一是 Feed 读取链路，使用游标分页、页缓存、卡片缓存、计数缓存、批量聚合和 singleflight 合并回源。第二是异步链路，发布视频、点赞收藏和曝光事件通过 RabbitMQ 解耦 API 和 Worker。第三是推荐与热榜，热榜使用 Redis ZSET 分钟桶，推荐采用热门/关注/向量三路并发召回（向量检索用 Milvus ANN），合并去重、配额保底、曝光剔除后精排融合兴趣相似度、热度和新鲜度，并做作者打散。
...
6. 读完第一页拿到 cursor 后，此时有新视频发布，翻第二页会看到这个新视频吗？

   答：不会。cursor 锁定了第一页最后一条的排序边界 `(published_at, video_id)`，第二页 SQL 条件是 `WHERE (published_at < cursor.PublishedAt OR (published_at = cursor.PublishedAt AND id < cursor.VideoID))`。新视频的 `published_at` 晚于 cursor 的时间锚点，落在游标"上方"，不在扫描范围内。这是游标分页的预期行为——翻页过程保持连续快照，不被头部插入干扰。用户回到顶部刷新（不带 cursor 重拉第一页）即可看到新视频。抖音、Twitter、微博的下拉翻页 + 顶部刷新都是这个模式。
...
7. 视频删除后，Feed 缓存里还能看到已删视频吗？怎么解决？

   答：能，但窗口被关闭了。删除是软删，只更新数据库状态，不清收件箱条目、不清卡片缓存——Feed 组装时卡片缓存命中就直接展示，所以删除后最长 15 分钟（卡片缓存 TTL）内 Feed 里还能看到已删视频，而详情页是实时 404。解决是 Redis 删除标记：删除落库成功后把视频 ID 写入标记集合（TTL 30 分钟，必须大于卡片缓存 TTL，否则标记先过期会重现窗口），Feed 组装时对所有条目批量校验，命中标记的直接剔除。标记是读侧统一过滤，时间线/热榜/关注/推荐四种 Feed 都经过同一个组装点，一处覆盖全部读取路径。两个降级：读标记失败就跳过过滤，退回旧行为，数据库状态过滤仍是兜底；写标记失败只记日志，删除照常成功。这套机制和未来的内容审核黑名单是同一个东西，做审核可以直接复用。
...
4. 一小时滑动热榜怎么生成？
   ...
   > **追问：60 个桶合并后排序具体怎么发生？** 写入阶段每个分钟桶内部已经是 ZSET（member=video_id, score=权重累加），`ZINCRBY` 只负责往当前分钟桶加分，不做排序。读取阶段 `ZUNIONSTORE ... AGGREGATE SUM` 把 60 个桶里相同 video_id 的 score 纵向求和，生成一个合并后的窗口 ZSET。最后 `ZREVRANGE` 按合并后的 score 从高到低取，天然就是热度排名。排序不是发生在桶之间，而是 `ZUNIONSTORE` 合并后的窗口 ZSET 上。
...
6. 如果用户关注了大量大 V，读关注流时需要拉取很多 outbox，怎么优化？

   答：当前实现中 `ListFollowingIndexPage` 会把 inbox 和所有关注大 V 的 outbox 通过一次 pipeline 并行拉取，关注几十个大 V 没问题。但关注上百个大 V 时 pipeline 命令数膨胀、内存 merge 量变大，且大部分 outbox 可能没有新内容但白拉了一次。优化方向：① 首屏只拉 inbox + 最近活跃的 Top 20 大 V outbox，凑够一页即返回，后续翻页再拉更多；② 写入侧对大 V 做选择性推送——虽然不推全部粉丝，但推给互动频繁的活跃粉丝，减少读时 outbox 依赖；③ 对合并后的关注流首页做短 TTL 缓存，关注关系变化时主动失效；④ 推拉结合精细化——不做按粉丝数一刀切，而是按"作者粉丝数 × 用户活跃度"二维矩阵决定 push 还是 pull，Twitter 当年的做法就是把活跃用户走 push、不活跃用户走 pull。
...
2. 推荐流为什么要做成多路召回？三路分别召回什么？

   答：单路热度召回有流行度偏差——冷启动新视频 hot 分低永远进不了候选池，且所有用户候选趋同、无个性化。所以拆成三路并发召回：热门路按 hot_score 倒序取全站最火；关注路取关注作者 72 小时内新视频（上限 200 条）；向量路把用户兴趣向量丢进 Milvus 做 ANN 检索，捞内容相似的长尾视频。三路互相独立，用 goroutine 并发执行，单路 500ms 超时，失败/超时的路静默返回空，不阻塞整体。最后用合并器统一去重（vector > following > hot 保留来源，热度分取最大值补齐）、关注路配额保底（池子至少 20%）、曝光剔除。

3. 推荐候选池大小怎么计算？

   答：候选池大小是 `limit * 8`，同时设置最小 50、最大 500。三路召回都按这个池大小执行（向量路 ANN 检索 topK 是池大小 ×3，上限 1500），合并器去重、配额、曝光剔除后凑成最终候选池。这样小页请求也有足够候选用于排序、过滤和作者打散，大页请求也会限制单次计算成本。代码常量是 `candidatePoolMultiplier`、`minCandidatePoolSize` 和 `maxCandidatePoolSize`。

4. 推荐排序用了哪些分数？

   答：推荐排序使用用户兴趣向量相似度、视频热度分和新鲜度分。新鲜度基于发布时间衰减，热度分用 `log1p` 平滑。向量路候选的相似度直接复用召回时 Milvus ANN 检索的距离换算，不必全池二次算向量；其他路候选在用户有向量时补算余弦相似度。最终 `RankScore` 决定排序，排序相同再按发布时间和 video_id 稳定兜底。

5. 个性化用户和冷启动用户的排序公式有什么区别？

   答：个性化用户有兴趣向量，排序公式是相似度 70%、热度 20%、新鲜度 10%。冷启动用户使用热度 65%、新鲜度 35%。召回侧同样有对应处理：没有用户向量时向量路自动跳过，退化为热门+关注双路召回。这样冷启动用户也能看到热门且较新的内容，同时推荐接口不会因为缺少向量而失败。
...
4. 如果用户兴趣向量为空，推荐排序如何兜底？
   ...
   答：召回侧向量路直接跳过（Milvus 查询需要用户向量），退化为热门+关注双路召回；排序侧使用热度和新鲜度排序，热度占 65%，新鲜度占 35%。这样冷启动用户会先看到近期热门内容。后续通过曝光、播放、完播和观看时长逐步构建用户兴趣向量，向量路自动恢复参与召回。
...
6. 如果推荐候选规模较小，应该如何补召回？
   ...
   答：三路召回已经覆盖热门、关注和向量三类来源，候选仍小通常意味着内容池本身有限或约束过严。可以调的旋钮：缩短向量路 30 天时间窗口、提高 ANN topK 上限（当前池大小 ×3，上限 1500）、放宽关注路 72 小时窗口、降低单路硬上限 200、缩短 7 天曝光去重窗口。监控上要关注推荐填充率和重复曝光率。

7. 向量路（Milvus）故障时推荐流如何降级？

   答：Milvus 通过 LazyStore 延迟初始化——后台重试连接和建集合，就绪前所有向量操作返回 ErrVectorStoreNotReady，向量路自动返回空，推荐退化为热门+关注双路，服务启动不被阻塞。单路查询超时（500ms context）也只丢弃该路结果。主链路精排始终有候选可排，这是"召回尽力而为、精排保底"的降级原则。
...
3. 推荐 API 测试应该覆盖哪些场景？
   ...
   答：覆盖多路召回、合并器去重与配额、排序分计算、个性化用户、冷启动用户、曝光去重、作者打散、cursor 翻页和候选规模较小的场景。还要测试单路召回挂起/失败时其余路正常返回（500ms 超时降级），以及向量路不可用时双路降级。另外 Milvus ANN 链路用 gated 集成测试（`TEST_MILVUS_ADDR` 未设置则跳过）验证距离排序和 COSINE 分数语义。还要测试 request_id、scene 和 limit 的参数校验。保存曝光需要验证同一请求内 video 去重。
...
1. Docker Compose 启动了哪些服务？
   ...
   答：Compose 启动 API、Worker、Web、MySQL、Redis、RabbitMQ、Milvus（含 etcd 和 MinIO 两个依赖）、Prometheus 和 Grafana。Web 通过 Vite/Nginx 提供前端，API 暴露业务接口和指标，Worker 消费异步任务。MySQL、Redis 和 RabbitMQ 提供核心基础设施，Milvus 提供视频向量检索。
...
7. 如果推荐结果重复率很高，你从哪些链路排查？
   ...
   答：先看曝光上报是否成功、曝光聚合表是否更新、推荐决策是否读取近期曝光。再看候选池是否过小、三路召回是否都在工作（某路静默降级会导致来源单一）、向量路数据是否完整（Milvus 回填进度、脏数据跳过）、作者打散是否生效。最后检查 cursor 分页和 request_id 是否正确传递。
